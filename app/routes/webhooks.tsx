...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  // Clone the request so we can read the body later if auth fails
  const clone = request.clone();

  try {
    const { topic, shop, session, admin, payload } = await authenticate.webhook(request);
    console.log(`[Webhook Success] Verified ${topic} for ${shop}`);

    // =================================================================
    // 1. GDPR & REDACTION LOGIC
    // =================================================================
    ...
    // [DEBUG] If HMAC fails, this log will now appear (it was hidden before)
    console.error(`[Webhook Error] Auth failed:`, error);
    
    // Log a snippet of the payload to prove we received data
    const text = await clone.text();
    console.log(`[Webhook Payload Snippet] ${text.slice(0, 50)}...`);
    
    // Return 401 so Shopify knows to try again later
